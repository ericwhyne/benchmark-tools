...
# host 3
ssh $user@$host3 "mkdir -p ~/.ssh"
cat temp-$keyfilename.key | ssh $user@$host3 "cat > ~/.ssh/id_rsa"
ssh $user@$host3 "chmod 600 ~/.ssh/id_rsa"
cat temp-$keyfilename.key.pub | ssh $user@$host3 "cat >> ~/.ssh/authorized_keys"
